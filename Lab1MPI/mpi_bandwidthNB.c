...
if (rank < numtasks/2) {
  for (n=start; n<=end; n=n+incr) {
    ...
    for (i=1; i<=rndtrps; i++){
      t1 = MPI_Wtime();
      MPI_Isend(&msgbuf, n, MPI_CHAR, dest, tag, MPI_COMM_WORLD, &rq[0]);
      MPI_Irecv(&msgbuf, n, MPI_CHAR, src, tag, MPI_COMM_WORLD, &rq[1]);
      MPI_Waitall(2, rq, stati);
      t2 = MPI_Wtime();
      thistime = t2 - t1;
